...
object LucroCategoria {
  
  def quebraBase(line : String)={
    val linhaQuebrada = line.split(";")
    (linhaQuebrada(13).toUpperCase().trim(), linhaQuebrada(19).replace(",", ".").trim().toFloat)
  }
  ...
  def main(args: Array[String]){
    ...
    //inverte os valores da tupla para (Lucro, Categoria)
    ...
    dataFrameProdutos.write.option("header", "false").csv("./exportacao/dataFrameLucroCategoria.csv")    

...
object QtdProdutoCategoria {
  
  def quebraBase(line : String)={
    val linhaQuebrada = line.toUpperCase().split(";")
    (linhaQuebrada(12).trim() + "," + linhaQuebrada(13).trim(), linhaQuebrada(17).trim().toInt)
  }
  ...
  def main(args: Array[String]){
    ...
    //inverte os valores da tupla para (Quantidade, "ID do produto,Categoria")
    ...
    dataFrameProdutos.write.option("header", "false").csv("./exportacao/dataFrameQtdProdutoCategoria.csv")    

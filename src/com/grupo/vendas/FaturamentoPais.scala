...
object FaturamentoPais {
  
  def quebraBase(line : String)={
    val linhaQuebrada = line.split(";")
    (linhaQuebrada(10).toUpperCase().trim(), linhaQuebrada(16).replace(",", ".").trim().toFloat)
  }
  ...
  def main(args: Array[String]){
    ...
    //inverte os valores da tupla para (Faturamento, País)
    ...
    dataFrameProdutos.write.option("header", "false").csv("./exportacao/dataFrameFaturamentoPais.csv")    

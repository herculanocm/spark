...
object LucroSubCategoriaPais {
  
  def quebraBase(line : String)={
    val linhaQuebrada = line.toUpperCase().split(";")
    (linhaQuebrada(13).trim() + "," + linhaQuebrada(14).trim() + "," + linhaQuebrada(10).trim(), linhaQuebrada(19).replace(",", ".").trim().toFloat)
  }
  ...
  def main(args: Array[String]){
    ...
    //inverte os valores da tupla para (Lucro, "Categoria,Subcategoria,País")
    ...
    dataFrameProdutos.write.option("header", "false").csv("./exportacao/dataFrameLucroSubCategoriaPais.csv")    

...
object LucroCategoriaRegiao {
  
    def quebraBase(line : String)={
    val linhaQuebrada = line.toUpperCase().split(";")
    ((linhaQuebrada(13).trim() + "," + linhaQuebrada(11).trim()), linhaQuebrada(19).replace(",", ".").trim().toFloat)
  }
  ...
  def main(args: Array[String]){
    ...
    //inverte os valores da tupla para (Lucro, "Categoria, Região")
    ...
    dataFrameProdutos.write.option("header", "false").csv("./exportacao/dataFrameLucroCategoriaRegiao.csv")    

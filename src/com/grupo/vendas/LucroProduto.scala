...
object LucroProduto {
  ...
  def quebraBase(line : String)={
    ...
    (linhaQuebrada(12).toUpperCase().trim(), linhaQuebrada(19).replace(",", ".").trim().toFloat)
  }
  ...
  def main(args: Array[String]){
    ...
    //inverte os valores da tupla para (Lucro, ID do produto)
    ...
    dataFrameProdutos.write.option("header", "false").csv("./exportacao/dataFrameLucroProdutos.csv")    
